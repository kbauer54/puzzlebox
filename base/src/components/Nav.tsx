...
export default function Nav({ currentPage, onNavigate, user, onLogin, onLogout }: NavProps) {
  ...
  const handleSignIn = () => {
    ...
    // Mock auth — in real app this hits your Spring Boot /api/users/login
    onLogin({ uid: Date.now(), name: form.email.split('@')[0], coins: 200, isBanned: false });
    setModal(null);
  ...
  const handleRegister = () => {
    ...
    // Mock register — in real app this hits your Spring Boot /api/users/register
    onLogin({ uid: Date.now(), name: form.name, coins: 200, isBanned: false });
    setModal(null);
    setForm({ name: '', email: '', password: '', confirm: '' });
    setError('');
  };
      ...
      <nav style={s.nav}>
        ...
        <div style={s.navLinks}>
          ...
          {user ? (
            ...
            <>
              <button style={s.btnGhost} onClick={() => { setError(''); setModal('signin'); }}>Sign In</button>
              <button style={s.btnPrimary} onClick={() => { setError(''); setModal('register'); }}>Register Free</button>
            </>
          )}
        </div>
      </nav>
        ...
        <div style={s.overlay} onClick={() => setModal(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()} className="pop-in">
            ...
            <p style={s.switchText}>
              No account?{' '}
              <span style={s.switchLink} onClick={() => { setError(''); setModal('register'); }}>
                Register free →
              </span>
            </p>
          </div>
      ...
      {modal === 'register' && (
        <div style={s.overlay} onClick={() => setModal(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()} className="pop-in">
            ...
            <p style={s.switchText}>
              Have an account?{' '}
              <span style={s.switchLink} onClick={() => { setError(''); setModal('signin'); }}>
                Sign in →
              </span>
            </p>
          </div>
