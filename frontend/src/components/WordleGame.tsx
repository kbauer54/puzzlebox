...
// ─── Component ───────────────────────────────────────────────────────────────
export default function WordleGame({ user, onScoreUpdate, onNavigate }: WordleGameProps) {
  ...
  // Keyboard state: best color per letter
  ...
  const submitGuess = useCallback(() => {
    ...
    if (won) {
      ...
      setTimeout(() => {
        setStatus('won');
        showToast(WIN_MESSAGES[newGuesses.length - 1] ?? 'Nice!', 2500);
        onScoreUpdate?.(score);
      }, WORD_LENGTH * 350 + 300);
  ...
  }, [current, guesses, answer, onScoreUpdate]);
  ...
  const reset = () => {
    ...
    setStatus('playing');    setRevealRow(null); setToast(''); setFinalScore(0);
  };
      ...
      <div style={s.wrapper}>
            ...
            return (
              <div key={rowIdx} style={{ ...s.row, animation: doShake ? 'wShake 0.5s ease' : undefined }}>
                {Array.from({ length: WORD_LENGTH }).map((_, colIdx) => {
                  ...
                  if (completed) {
                    letter = completed[colIdx].letter;
                    state  = completed[colIdx].state;
                  } else if (isCurrent) {
        ...
        {status !== 'playing' && (
          <div style={s.endPanel} className="pop-in">
            <div style={s.endEmoji}>{status === 'won' ? '🏆' : '😔'}</div>
            {status === 'won' ? (
              <>
                <div style={s.endScore}>{finalScore} pts</div>
                <div style={s.endSub}>Solved in {guesses.length} {guesses.length === 1 ? 'guess' : 'guesses'}</div>
              </>
            ) : (
              <>
                <div style={s.endScore}>Better luck next time</div>
                <div style={s.endSub}>The word was <strong style={{ color: 'var(--yellow)' }}>{answer}</strong></div>
              </>
            )}
